...
class GarchModel:
    ...
    def dump(self):
        ...
        # Create timestamp and replace invalid characters
        timestamp = pd.Timestamp.now().isoformat()
        timestamp_safe = re.sub(r"[:]", "-", timestamp)  # Replace colons with dashes

        # Create filepath, including `self.model_directory`
        filepath = os.path.join(self.model_directory, f"{timestamp_safe}_{self.ticker}.pkl")
